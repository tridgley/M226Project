...
class _BaseHMM(BaseEstimator):
    ...
    def _score_log(self, X, lengths=None, *, compute_posteriors):
        ...
        *compute_posteriors* is True (otherwise, an empty array is returned
        ...
        for sub_X in _utils.split_X_lengths(X, lengths):
            ...
            logprobij, fwdlattice, _ = self._do_forward_log_pass(framelogprob)
            logprob += logprobij
            if compute_posteriors:
                bwdlattice, _ = self._do_backward_log_pass(framelogprob)
    ...
    def _decode_viterbi(self, X, si):
        framelogprob = self._compute_log_likelihood(X)
        #if si == 3:
        #_log.warning("INFO <hmmbase.py> _decode_viterbi: chr %d framelogprob", si)
        #_log.warning(np.shape(framelogprob))
        #_log.warning(framelogprob)
    ...
    def decode(self, X, lengths=None, algorithm=None):
        ...
        for sub_X in _utils.split_X_lengths(X, lengths):
            ...
            _log.warning("INFO <hmmbase.py> decode() sub_X %d iteration", counter)
            sub_logprob, sub_state_sequence = decoder(sub_X, counter)
            logprob += sub_logprob
    ...
    def fit(self, X, lengths=None):
        ...
        for iter in range(self.n_iter):
            ...
            curr_logprob = 0
            for sub_X in _utils.split_X_lengths(X, lengths):
                lattice, logprob, posteriors, fwdlattice, bwdlattice, fwd_segments, bw_segments = \
                        impl(sub_X)
                ...
                self._accumulate_sufficient_statistics(
                    ...
                    bwdlattice, fwd_segments, bw_segments)
    ...
    def _fit_log(self, X):
        framelogprob = self._compute_log_likelihood(X)
        logprob, fwdlattice, fwd_segments = self._do_forward_log_pass(framelogprob)
        bwdlattice, bw_segments = self._do_backward_log_pass(framelogprob)
        posteriors = self._compute_posteriors_log(fwdlattice, bwdlattice)
        return framelogprob, logprob, posteriors, fwdlattice, bwdlattice, fwd_segments, bw_segments

    def _do_viterbi_pass(self, framelogprob):
        state_sequence, logprob = _hmmc.viterbi(
            log_mask_zero(self.startprob_), log_mask_zero(self.transmat_),
            framelogprob, self.smax)
        return logprob, state_sequence
    ...
    def _do_forward_log_pass(self, framelogprob):
        fwdlattice, fwd_segments = _hmmc.forward_log(
            log_mask_zero(self.startprob_), log_mask_zero(self.transmat_),
            framelogprob, self.smax)
        with np.errstate(under="ignore"):
            return special.logsumexp(fwdlattice[-1]), fwdlattice, fwd_segments
    ...
    def _do_backward_log_pass(self, framelogprob):
        bwdlattice, bw_segments = _hmmc.backward_log(
            log_mask_zero(self.startprob_), log_mask_zero(self.transmat_),
            framelogprob, self.smax)
        return bwdlattice, bw_segments
    ...
    def _initialize_sufficient_statistics(self):
        ...
        stats = {'nobs': 0,
                 ...
                 'trans': np.zeros((self.n_components, self.n_components), dtype=np.float128)}
        return stats

    def _accumulate_sufficient_statistics(self, stats, X, lattice,
                                          posteriors, fwdlattice, bwdlattice, fwd_segments, bw_segments):
        ...
        return impl(stats=stats, X=X, lattice=lattice, posteriors=posteriors,
                    fwdlattice=fwdlattice, bwdlattice=bwdlattice, fwd_segments=fwd_segments, bw_segments=bw_segments)
    ...
    def _accumulate_sufficient_statistics_log(self, stats, X, lattice,
        ...
        if 't' in self.params:
            ...
            log_xi_sum = _hmmc.compute_log_xi_sum(
                fwdlattice, log_mask_zero(self.transmat_), bwdlattice, lattice, fwd_segments, bw_segments, self.smax)
            with np.errstate(under="ignore"):
                #print("INFO: log_xi_sum", type(log_xi_sum), log_xi_sum)
                stats['trans'] += np.exp(log_xi_sum.astype(np.float128))
